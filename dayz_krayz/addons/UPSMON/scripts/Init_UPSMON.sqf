...
//Adding eventhandlers
	"KRON_UPS_EAST_SURRENDED" addPublicVariableEventHandler { if (_this select 1) then { nul=[east] execvm "\dayz_krayz\addons\UPSMON\scripts\UPSMON\MON_surrended.sqf";};};
	"KRON_UPS_WEST_SURRENDED" addPublicVariableEventHandler { if (_this select 1) then { nul=[west] execvm "\dayz_krayz\addons\UPSMON\scripts\UPSMON\MON_surrended.sqf";};};
	"KRON_UPS_GUER_SURRENDED" addPublicVariableEventHandler { if (_this select 1) then { nul=[resistance] execvm "\dayz_krayz\addons\UPSMON\scripts\UPSMON\MON_surrended.sqf";};};
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	if (isNil("KRON_UPS_INIT") || KRON_UPS_INIT == 0) then {
		...
		call compile preprocessFileLineNumbers "\dayz_krayz\addons\UPSMON\scripts\UPSMON\common\MON_functions.sqf";
		
		//init !R functions
		call compile preprocessFileLineNumbers "\dayz_krayz\addons\UPSMON\scripts\UPSMON\!R\R_functions.sqf";
		
		if (isNil "RE") then {[] execVM "\ca\Modules\MP\data\scripts\MPframework.sqf"}; 
		
		//scripts initialization
		UPSMON = compile preprocessFile "\dayz_krayz\addons\UPSMON\scripts\UPSMON.sqf";	
		UPSMON_surrended = compile preprocessFile "\dayz_krayz\addons\UPSMON\scripts\UPSMON\MON_surrended.sqf";	
