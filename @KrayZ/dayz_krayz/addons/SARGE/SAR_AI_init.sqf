...
for [{_i=0}, {_i < _gridsize_y}, {_i=_i+1}] do
{
    for [{_ii=0}, {_ii < _gridsize_x}, {_ii=_ii+1}] do
    {
        ...
        if (SAR_dynamic_spawning) then {
            ...
            _trig_act_stmnt = format["if (SAR_DEBUG) then {diag_log 'SAR DEBUG: trigger on in %1';};[thislist,'%1'] execVM'\dayz_krayz\addons\SARGE\SAR_AI_spawn.sqf';",_triggername];
            _trig_deact_stmnt = format["if (SAR_DEBUG) then {diag_log 'SAR DEBUG: trigger off in %1';};[thislist,thisTrigger,'%1'] execVM'\dayz_krayz\addons\SARGE\SAR_AI_despawn.sqf';",_triggername];
...
[] execVM "\dayz_krayz\addons\SARGE\SAR_group_monitor.sqf";
...
if(SAR_FIX_VEHICLE_ISSUE) then {
    ...
    _script_handler = [] execVM "\dayz_krayz\addons\SARGE\SAR_vehicle_fix.sqf";
